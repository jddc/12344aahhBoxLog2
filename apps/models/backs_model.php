<?php
	class Backs_model extends Model{
		private $allow_null = array(
                    'details'
                );
                
		public function get_order($id_sap_order = ""){
			$this -> db -> get("select id_sap_order,state,details,date from backs where id_sap_order = ".$id_sap_order);
			return $this -> db -> dbs_rows;
		}

		public function get_orders($limit){	
			$this -> db -> get("select id_sap_order,state,details,date from backs limit ".$limit);
			return $this -> db -> dbs_rows;
		}
